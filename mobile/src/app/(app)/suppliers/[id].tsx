...
interface SpendEntry { month: string; totalGbp: number; }
...
export default function SupplierDetailScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      suppliersApi.get(id),
      suppliersApi.getProducts(id),
      suppliersApi.getSpend(id),
    ])
      .then(([s, p, sp]) => {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.meta}>{supplier.supplierType} · {supplier.city ?? supplier.country}</Text>
      ...
      <View style={styles.section}>
        ...
        {products.map((p) => (
          <TouchableOpacity key={p.id} style={styles.chip} onPress={() => router.push(`/(app)/products/${p.id}`)}>
...
const styles = StyleSheet.create({
  ...
  editButton: { marginTop: 12, alignSelf: 'flex-start', borderWidth: 1, borderColor: '#2563eb', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 6 },
  ...
  chip: { backgroundColor: '#fff', borderRadius: 8, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: '#e5e7eb', flexDirection: 'row', justifyContent: 'space-between' },
  chipText: { fontSize: 14, color: '#111827', fontWeight: '500' },
  chipSub: { fontSize: 12, color: '#9ca3af' },
  spendRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderColor: '#f3f4f6' },
