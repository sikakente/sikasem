...
export default function EditSupplierScreen() {
  ...
  useEffect(() => {
    ...
    suppliersApi.get(id).then((res) => {
      ...
      setForm({
        name: s.name ?? '', supplierType: s.supplierType ?? 'shop',
        contactName: s.contactName ?? '', phone: s.phone ?? '',
        email: s.email ?? '', addressLine1: s.addressLine1 ?? '',
        city: s.city ?? '', country: s.country ?? '',
        currencyCode: s.currencyCode ?? 'GBP', notes: s.notes ?? '',
      });
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        {([['name', 'Name *'], ['contactName', 'Contact Name'], ['phone', 'Phone'], ['email', 'Email'], ['addressLine1', 'Address'], ['city', 'City'], ['country', 'Country *'], ['currencyCode', 'Currency'], ['notes', 'Notes']] as [string, string][]).map(([key, label]) => (
          <View key={key}>
